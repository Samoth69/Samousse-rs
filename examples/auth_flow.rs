//! src: https://github.com/twitch-rs/twitch_oauth2/blob/main/examples/auth_flow.rs
...
async fn main() -> anyhow::Result<()> {
    ...
    // Create the builder!
    let mut builder =
        UserTokenBuilder::new(client_id, client_secret, redirect_url).force_verify(false);
    ...
    match (map.get("state"), map.get("code")) {
        (Some(state), Some(code)) => {
            ...
            println!("AccessToken: {}", token.access_token.secret());
            println!("ClientToken: {}", token.client_id().as_str());
            println!("RefreshToken: {}", token.refresh_token.unwrap().secret());
        }
